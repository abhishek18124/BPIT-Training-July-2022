...
int minCoins(int n, vector<int>& c) {
	...
	for(int j=0; j<k; j++) {
		if(n-c[j] >= 0) {
			// you can choose the coin with denomination c[j] to reduce n to n-c[j]
...
int minCoinsTopDown(int n, vector<int>& c, vector<int>& dp) {
	...
	// lookup
	if(dp[n] != -1) {
		// you already known the min. no. of coins required to 
		// reduce n to 0
		return dp[n];
	}
...
int minCoinsBottomUp(int n, vector<int> c) {
	...
	return dp[n]; // dp[n] = f(n) -> min. coins required to reduce n to 0 : original problem
...
int main() {
	...
	ans == INT_MAX ? cout << "we cannot change the amount" << endl :
	                 cout << ans << endl;

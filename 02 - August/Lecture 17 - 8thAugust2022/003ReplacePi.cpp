...
void replacePi(char* inp, int i) {
	...
	if(inp[i] == 'p' and inp[i+1] == 'i') {
		...
		// 2. replace (i, i+1, i+2, i+3) characters with "3.14"
		inp[i] = '3';
		inp[i+1] = '.';
		inp[i+2] = '1';
		inp[i+3] = '4';

		// 3. ask your friend to replace all the "pi"s with "3.14" in the inp[i+4...n-1]
		replacePi(inp, i+4);

	} else {
		// ask your friend to replace all the "pi"s with "3.14" in the inp[i+1...n-1]
		replacePi(inp, i+1);
